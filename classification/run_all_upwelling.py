...
from make_plots import multiple_upwelling_pr_re_plot, multiple_upwelling_roc_auc_plot, multiple_upwelling_window_errors
...
for z in zones:
    ...
    for file_name in os.listdir(folder_path):
        ...
        window_errors_dsp, y_real_dsp, y_pred_dsp, proba_dsp = run_svm_class(df_drop, file_name_shortened, df_year,
                                                                            None, title)
    ...
    for file_name in os.listdir(folder_path):
        ...
        df_year = df_pre['instance'].str[:4].rename('year').to_frame()
        df_drop = df_pre.drop(['date', 'ESTACAO', 'instance'], axis=1)
        window_errors_hydro, y_real_hydro, y_pred_hydro, proba_hydro = run_svm_class(df_drop, file_name_shortened,
                                                                                    df_year, None,
                                                                                    title)
    ...
    for file_name in os.listdir(folder_path):
        ...
        df_year = df_pre['instance'].str[:4].rename('year').to_frame()
        df_drop = df_pre.drop(['date', 'ESTACAO', 'instance'], axis=1)
        window_errors_hydro_upwelling, y_real_hydro_upwelling, y_pred_hydro_upwelling, proba_hydro_upwelling = run_svm_class(
            df_drop,
            file_name_shortened,
            df_year, None,
            title)
    ...
    for file_name in os.listdir(folder_path):
        ...
        window_errors_meteo, y_real_meteo, y_pred_meteo, proba_meteo = run_svm_class(df_drop, file_name_shortened,
                                                                                    df_year, None,
                                                                                    title)
    ...
    for file_name in os.listdir(folder_path):
        ...
        df_year = df_pre['instance'].str[:4].rename('year').to_frame()
        df_drop = df_pre.drop(['date', 'ESTACAO', 'instance', 'year'], axis=1)
        window_errors_meteo_hydro, y_real_meteo_hydro, y_pred_meteo_hydro, proba_meteo_hydro = run_svm_class(df_drop,
                                                                                                            file_name_shortened,
                                                                                                            df_year,
                                                                                                            None,
                                                                                                            title)
    ...
    for file_name in os.listdir(folder_path):
        ...
        df_year = df_pre['instance'].str[:4].rename('year').to_frame()
        df_drop = df_pre.drop(['date', 'ESTACAO', 'instance', 'year'], axis=1)
        window_errors_meteo_hydro_upwelling, y_real_meteo_hydro_upwelling, y_pred_meteo_hydro_upwelling, proba_meteo_hydro_upwelling = run_svm_class(
            df_drop, file_name_shortened, df_year, None, title)
    ...
    for file_name in os.listdir(folder_path):
        ...
        if z in ['l5b_Caparica']:
            df_drop = df_drop.drop(['PR_DUR'], axis=1)
        window_errors_meteo_upwelling, y_real_meteo_upwelling, y_pred_meteo_upwelling, proba_meteo_upwelling = run_svm_class(
            df_drop,
            file_name_shortened,
            df_year, None,
            title)
    ...
    for file_name in os.listdir(folder_path):
        ...
        df_year = df_pre['instance'].str[:4].rename('year').to_frame()
        df_drop = df_pre.drop(['date', 'ESTACAO', 'instance'], axis=1)
        window_errors_dsp_upwelling, y_real_dsp_upwelling, y_pred_dsp_upwelling, proba_dsp_upwelling = run_svm_class(
            df_drop,
            file_name_shortened,
            df_year, None, title)
    ...
    multiple_upwelling_roc_auc_plot(y_real_dsp, y_pred_dsp, y_real_meteo, y_pred_meteo, y_real_hydro, y_pred_hydro,
                                    y_real_meteo_hydro,
                                    y_pred_meteo_hydro, y_real_dsp_upwelling, y_pred_dsp_upwelling,
                                    y_real_meteo_upwelling, y_pred_meteo_upwelling,
                                    y_real_hydro_upwelling, y_pred_hydro_upwelling, y_real_meteo_hydro_upwelling,
                                    y_pred_meteo_hydro_upwelling, f'{z}_upwelling_{alg_type}', proba_dsp, proba_meteo,
                                    proba_hydro, proba_meteo_hydro, proba_dsp_upwelling, proba_meteo_upwelling,
                                    proba_hydro_upwelling, proba_meteo_hydro_upwelling)
    multiple_upwelling_pr_re_plot(y_real_dsp, y_pred_dsp, y_real_meteo, y_pred_meteo, y_real_hydro, y_pred_hydro,
                                  y_real_meteo_hydro,
                                  y_pred_meteo_hydro, y_real_dsp_upwelling, y_pred_dsp_upwelling,
                                  y_real_meteo_upwelling, y_pred_meteo_upwelling,
                                  y_real_hydro_upwelling, y_pred_hydro_upwelling, y_real_meteo_hydro_upwelling,
