...
def train_test_split_mix(dsp_std, window_size, feature_names_w, dsp_zone):
    inputs_df = pd.DataFrame(columns=feature_names_w)
    dsp_std_test = dsp_std.loc[dsp_zone[dsp_zone['assigned'] == 'L1 CARREÇO'].index]
    print(dsp_std_test)
    dsp_std = dsp_std.loc[dsp_zone[dsp_zone['assigned'] != 'L1 CARREÇO'].index]
    print(dsp_std)
    dsp_zone = dsp_zone[dsp_zone['assigned'] != 'L1 CARREÇO']
    print(dsp_zone)
    dsp_std_test = dsp_std_test.reset_index(drop=True)
    dsp_std = dsp_std.reset_index(drop=True)
    dsp_zone = dsp_zone.reset_index(drop=True)
    outputs_df = pd.DataFrame(columns=['DSP_out'])
    dsp_cols = [col for col in dsp_std.columns if 'DSP' not in col]
    dsp_std[dsp_cols] = dsp_std[dsp_cols].replace(np.nan, -990.0)
    for i in range(window_size, len(dsp_std)):
        inputs = dsp_std.iloc[i - window_size:i]
        output = dsp_std[['DSP']].iloc[i]
        inputs_zone = dsp_zone.iloc[i - window_size:i]
        output_zone = dsp_zone.iloc[i]
        if inputs.isnull().values.any() or output.isnull().values.any() or inputs_zone['assigned'].nunique() != 1 or \
                inputs_zone['assigned'].iloc[0] != output_zone['assigned']:
            continue
        else:
            new_inputs = inputs.values.reshape(1, -1)
            # Convert the array to a DataFrame
    ...
    # NEW
    inputs_df_test = pd.DataFrame(columns=feature_names_w)
    outputs_df_test = pd.DataFrame(columns=['DSP_out'])
    dsp_std_test[dsp_cols] = dsp_std_test[dsp_cols].replace(np.nan, -990.0)
    for i in range(window_size, len(dsp_std_test)):
        inputs = dsp_std_test.iloc[i - window_size:i]
        output = dsp_std_test[['DSP']].iloc[i]
        if inputs.isnull().values.any() or output.isnull().values.any():
            continue
        else:
            new_inputs = inputs.values.reshape(1, -1)
            # Convert the array to a DataFrame
            temp_df = pd.DataFrame(new_inputs, columns=inputs_df_test.columns)
            inputs_df = pd.concat([inputs_df_test, temp_df], ignore_index=True)

            temp_df = pd.DataFrame(output.values, columns=outputs_df_test.columns)
            outputs_df_test = pd.concat([outputs_df_test, temp_df], ignore_index=True)

    dsp_input_cols = [col for col in inputs_df_test.columns if 'DSP' not in col]
    inputs_df_test[dsp_input_cols] = inputs_df_test[dsp_input_cols].replace(-990.0, np.nan)
    ...
    # Concatenate the dataframes
    inputs_df = pd.concat([inputs_df, outputs_df], axis=1)
    inputs_df_test = pd.concat([inputs_df_test, outputs_df_test], axis=1)
    #test_size = round(inputs_df.shape[0] * 0.2)
    train_inputs = inputs_df.copy()
    dsp_diff, dsp_min = 0, 0
    test_inputs = inputs_df_test.copy()
    for string in strings_to_match:
        matching_columns = [col for col in train_inputs.columns if string in col]
        train_subset = train_inputs.loc[:, matching_columns]
        test_subset = test_inputs.loc[:, matching_columns]
