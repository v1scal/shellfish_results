...
from make_plots import cm_plot, class_predictions_plot, window_errors, param_plot, metric_plot, fi_plot, \
    generate_test_metrics_class, generate_best_params, roc_auc_plot, pr_re_plot, window_errors_2
from sklearn.metrics import make_scorer
from imblearn.metrics import geometric_mean_score


def run_svm_class(df, prefix_dsp, df_year, df_assigned, title):
    dsp_std = df
    prefix = prefix_dsp
    ...
    for w_id, window_size in enumerate(window_sizes):
        ...
        # df_results.to_csv('uwu.csv', index=False)
    ...
    best_rf = SVC(**window_best_params, class_weight='balanced', probability=True)
    ...
    # A is real output, B is predicted, C is misclassifications, D is class changes, E is misclassifications during class changes, F is correct classifications during class change and G is misclassifications not during state changes
